        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input v-model.number="year" type="number" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500" />
            </div>
            <div>
            ...
            <table class="min-w-full text-sm">
              <thead class="sticky top-0 bg-gray-50 dark:bg-gray-900 z-10">
                <tr class="text-left border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th @click="toggleSort('m')" class="py-2 px-2 cursor-pointer select-none">Mjesec <SortIcon :active="sortKey==='m'" :dir="sortDir" /></th>
              ...
              <tbody>
                <tr v-for="r in displayedRows" :key="`${r.y}-${r.m}`" class="border-b border-gray-100 dark:border-gray-700 odd:bg-gray-50/50 dark:odd:bg-white/5 hover:bg-indigo-50/40 dark:hover:bg-indigo-900/20">
...
function toggleSort(key){
  ...
  else { sortKey.value = key; sortDir.value = key==='m' || key==='y' ? 'asc' : 'desc'; }
}
...
function exportCsv(){
  const headers = ['Godina','Mjesec','Broj naloga','Ukupna kolicina'];
  const lines = displayedRows.value.map(r => [r.y, r.m, r.orders_count, r.total_quantity]);
  const csv = [headers, ...lines].map(row => row.map(v => `"${(v??'').toString().replace(/"/g,'""')}"`).join(',')).join('\r\n');
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `izvjestaj-mjesecni-${year.value}.csv`; a.click();
