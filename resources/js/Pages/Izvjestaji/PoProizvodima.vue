      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10">
          <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
              ...
              <input v-model="from" type="date" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500" />
            </div>
            <div>
              <label class="block text-xs text-gray-500">Do datuma</label>
              <input v-model="to" type="date" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500" />
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10">
            <div class="text-xs text-gray-500">Broj naloga</div>
            ...
            <table class="min-w-full text-sm">
              <thead class="sticky top-0 bg-gray-50 dark:bg-gray-900 z-10">
                <tr class="text-left border-b border-gray-200 dark:border-gray-700">
                  <th @click="toggleSort('product_name')" class="py-2 px-2 cursor-pointer select-none">Proizvod <SortIcon :active="sortKey==='product_name'" :dir="sortDir" /></th>
              ...
              <tbody>
                <tr v-for="r in displayedRows" :key="r.product_id ?? r.product_name" class="border-b border-gray-100 dark:border-gray-700 odd:bg-gray-50/50 dark:odd:bg-white/5 hover:bg-indigo-50/40 dark:hover:bg-indigo-900/20">
...
function toggleSort(key){
  ...
  else { sortKey.value = key; sortDir.value = key==='product_name' ? 'asc' : 'desc'; }
}
...
function resetFilters(){
  ...
  from.value = lastYear.toISOString().slice(0,10);
  status.value = 'Odobreno';
  reload();
}

function exportCsv(){
  const headers = ['Proizvod','Broj naloga','Ukupna kolicina'];
  const lines = displayedRows.value.map(r => [r.product_name, r.orders_count, r.total_quantity]);
  const csv = [headers, ...lines].map(row => row.map(v => `"${(v??'').toString().replace(/"/g,'""')}"`).join(',')).join('\r\n');
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `izvjestaj-proizvodi.csv`; a.click();
