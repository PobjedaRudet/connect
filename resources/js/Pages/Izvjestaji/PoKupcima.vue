            ...
            <table class="min-w-full text-sm">
              <thead class="sticky top-0 bg-gray-50 dark:bg-gray-900 z-10">
                <tr class="text-left border-b border-gray-200 dark:border-gray-700">
                  <th @click="toggleSort('partner_name')" class="py-2 px-2 cursor-pointer select-none">Kupac <SortIcon :active="sortKey==='partner_name'" :dir="sortDir" /></th>
              ...
              <tbody>
                <tr v-for="r in displayedRows" :key="r.partner_id ?? r.partner_name" class="border-b border-gray-100 dark:border-gray-700 odd:bg-gray-50/50 dark:odd:bg-white/5 hover:bg-indigo-50/40 dark:hover:bg-indigo-900/20">
...
function toggleSort(key){
  ...
  else { sortKey.value = key; sortDir.value = key==='partner_name' ? 'asc' : 'desc'; }
}
...
function exportCsv(){
  const headers = ['Kupac','Broj naloga','Ukupna kolicina'];
  const lines = displayedRows.value.map(r => [r.partner_name, r.orders_count, r.total_quantity]);
  const csv = [headers, ...lines].map(row => row.map(v => `"${(v??'').toString().replace(/"/g,'""')}"`).join(',')).join('\r\n');
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `izvjestaj-kupci.csv`; a.click();
